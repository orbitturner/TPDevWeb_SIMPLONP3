...
if (isset($_POST['FormAddAccountVALIDATOR']) && !(empty($_POST['typeAccount'])) && in_array($_POST['typeAccount'], $accountTypesValidate) == true && $_POST['typeAccount'] == 'cesp') {
    ...
    $idActualAgency = 1;
    $accountNumber = accountNumGen($idActualAgency);
    $state = 1;
    $typeClientOwner = 1;
    // var_dump($accountNumber);
    // INSERTING
    $row = persistCompteEPSX($accountNumber,$cleRIB,$idOwnerCompte, $soldeAccount, $state, $idConnectedUser,$numAgency, $accountCreationFee, $nextRemunDate,$typeClientOwner);
